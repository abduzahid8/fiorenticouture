...
const formatPrice = (price) => {
  if (price === null || price === undefined || typeof price !== 'number') {
    return 'Price unavailable'; // Or return an empty string, or '0'
  }
              ...
              <h3 className="text-sm mb-2">{product.name || 'Unnamed Product'}</h3>
                  <p className="text-gray-500 font-medium text-sm tracking-tighter">
                    {typeof product.price === 'number' ?
                      `${formatPrice(product.price)} UZS` : // Если цена - число, форматируем и добавляем UZS
                      'Price unavailable' // В противном случае (если не число), выводим "Price unavailable"
                    }
                  </p>
            </div>
          </Link>
